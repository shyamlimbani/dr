const generateHtmlTemplate = (type, data, settings) => {
  ...
  const docNumber = isBill ? data.billNumber : data.quotationNumber;
  const docDate = isBill ? data.billDate : data.quotationDate;
  const dueDate = data.eventDate || docDate; // Default due date to event date

  let status = isBill ? 'Pending' : 'Draft';
  let statusBadge = '';
  ...
  if (isBill) {
    ...
    statusBadge = '<span class="px-3 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-700 border border-blue-200">QUOTATION</span>';
  }

  const primaryColor = isBill ? '#0f766e' : '#0369a1'; // Teal for Bill, Sky Blue for Qtn
  const bgAccent = isBill ? 'bg-teal-50' : 'bg-sky-50';
  const textAccent = isBill ? 'text-teal-700' : 'text-sky-700';
    ...
    <div class="absolute bottom-[40px] left-[50px] right-[50px]">
      ...
      <div class="flex justify-between items-end border-t border-slate-200 pt-8">
        <div>
          ...
          <p class="text-xs text-slate-500">${isBill ? 'Payment is due within 15 days of issue.' : 'Terms & Conditions: Validity of this quotation is 30 days.'}</p>
        </div>
