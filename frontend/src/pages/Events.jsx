...
const Events = () => {
  ...
  const [tempFromDate, setTempFromDate] = useState(getFirstDayOfMonth());
  const [tempToDate, setTempToDate] = useState(getLastDayOfMonth());
  ...
  const downloadMonthlyPdf = async () => {
    try {
      ...
      const filename = `Employee_Report_${formatDate(fromDate)}_to_${formatDate(toDate)}.pdf`;

      const htmlContent = getEmployeeMonthlyReportHtml({
        employee: activeEmployee,
        reportMonth: `${formatDate(fromDate)} to ${formatDate(toDate)}`,
  ...
  const filteredEvents = employeeEvents.filter(ev => {
    ...
    return eventDateStr >= fromDate && eventDateStr <= toDate;
  });
  ...
  const filteredPayments = employeeLedgers.filter(p => {
    ...
    return paymentDateStr >= fromDate && paymentDateStr <= toDate;
  });
        ...
        <div className="space-y-8 animate-in fade-in duration-300 max-w-5xl mx-auto pb-12">
          ...
          <div className="space-y-6">
            <div className="border-b border-slate-100 dark:border-slate-800/60 pb-3 flex flex-col md:flex-row md:justify-between md:items-end gap-4">
              <div>
                <h3 className="text-2xl font-black text-slate-800 dark:text-white tracking-tight">Performance Report</h3>
                <p className="text-sm text-slate-500 mt-1">Track employee assignments and payment history by date range.</p>
              </div>
              <div className="flex items-center gap-3 flex-wrap">
                <div className="flex items-center gap-2">
                  ...
                  <input 
                    ...
                    className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-sm font-semibold text-slate-700 dark:text-slate-300 focus:outline-none focus:border-indigo-500"
                  />
                </div>
                <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-indigo-500 hover:bg-indigo-400 text-white font-bold rounded-xl shadow-md transition-colors text-sm"
                >
                  Apply Filter
                </button>
                <button
