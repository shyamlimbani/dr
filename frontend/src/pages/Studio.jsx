                ...
                {filteredBookings.map(b => (
                  <tr key={b._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4 flex items-center justify-end gap-2">
                      <button 
                        onClick={() => window.open(getWhatsAppUrl(b.mobileNumber, getBookingWhatsAppMessage(b)), '_blank')}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-800 flex flex-col gap-3 bg-slate-50 dark:bg-slate-950 shrink-0">
                ...
                {editingId && (
                  <button
                    type="button"
                    onClick={() => {
                      const b = bookings.find(bk => bk._id === editingId);
                      if (b) window.open(getWhatsAppUrl(b.mobileNumber, getBookingWhatsAppMessage(b)), '_blank');
                    }}
                    className="w-full h-12 flex items-center justify-center gap-2 bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366]/20 font-bold rounded-2xl transition-colors text-sm"
                  >
                    📱 Send WhatsApp
                  </button>
                )}
